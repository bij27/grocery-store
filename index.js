...
function getItemPriceByName(items, groceryName) {
  ...
  // const retrievedItem = items.find((item) => item.name === groceryName);
  // return retrievedItem.price;
}
 ...
 */
function getItemsByCategory(items, groceryCategory) {
  const results = items.filter(
    (groceryItem) => groceryItem.category === groceryCategory
  );
  console.log(results);
}
...
function countItems(items) {
  const totalQuantity = items.reduce(
    (totalQuantity, currentItem) => (total += currentItem.quantity),
    0
  );
...
const totalPrice = items.reduce(
  (totalValue, currentItem) => totalValue + currentItem.quantity * currentPrice,
  0
);
return totalPrice;
